...
If this returns nothing or errors, the Atlassian connector isn't connected. Stop and
tell the user to connect it — there is no fallback path.
...
Create `~/.claude/jira-automation/config.json`:
...
write what step 3 actually returned, never what this example shows.
...
Tell the user what you found and where you saved it. Keep it to a few lines — the site,
the projects, and anything surprising (a missing priority field, an unusual type
spelling). This is also the moment to ask whether one project should be the default.
...
Same steps 2–3 for the one project, then merge its entry into `projects`. No need to
